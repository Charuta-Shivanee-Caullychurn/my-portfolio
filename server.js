...
async function sendContactEmail(payload) {
  ...
  if (!response.ok) {
    ...
    const error = new Error(
      `Mail provider returned ${response.status}: ${providerMessage.slice(0, 300)}`,
    );
...
async function handleContact(request, response) {
  try {
    ...
    if (validationError)
      return sendJson(response, 400, { ok: false, error: validationError });
